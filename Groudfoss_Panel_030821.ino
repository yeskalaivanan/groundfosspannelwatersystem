...
int total_motor_count = 3;
int VFD_MOTOR[3] = {4, 6, 7};
//int VFD_MOTOR[3] = {4, 5, 7};
int AC_MOTOR[3] = {8, 10, 11};
...
void Read_Pressure()
{
  ...
  Read_Current_ADC = ((ReadADC * 5.0) / 1024);
...
{    
    MotorAllOff();//Switch off All motors  
    lcd.clear();   
    if (Select_Master >= total_motor_count)
    {
      Select_Master = 0;        
    }
    if (Select_Master == 0)
  ...
  {       
    ...
    else if (VFD_DAC <= 400 and MOTOR2_STATUS == HIGH)// and MOTOR3_STATUS == HIGH and MOTOR4_STATUS == HIGH)
    {      
      digitalWrite(MOTOR2, HIGH);
      delay(500);
      MOTOR2_STATUS = LOW;                
     ...
     else if (VFD_DAC <= 400 and MOTOR3_STATUS == HIGH)// and MOTOR3_STATUS == HIGH and MOTOR4_STATUS == HIGH)
    {      
      digitalWrite(MOTOR3, HIGH);
      delay(500);
      MOTOR3_STATUS = LOW;   
